...
from learn_tools.learner import SEPARATOR, SCORE, THRESHOLD
...
from learn_tools.active_bnn import BNN_MODELS, CLASSIFIERS
...
def active_learning(learner, num, discrete_feature=False, random_feature=False, include_none=True, visualize=False):
    ...
    while len(online) < num:
        ...
        attempts += 1
        # type of query used in active learning;
...
def optimize_index(learner, select_data, indices, noise=False):
    ...
    if alg_name in (BATCH_STRADDLE_ACTIVE+BNN_MODELS):
...
def learning_discrete(learner, select_data, num=1, random_feature=False):
    online = []
    if not learner.trained:
    ...
    if 'AL' in learner.algorithm.label:
        while len(online) < num:
            print(SEPARATOR)
            # new example
            # No need to change learner.query_type
            if random_feature:
        ...
        while len(online) < num:
            ...
            idx = np.random.randint(0, len(select_data))
            print('Acquisition: {} | {}/{} passive samples | {} candidates | selected index: {}'.format(
                alg_name, len(online), num, len(indices), idx)) # TODO: print the score
            #idx = learner.select_index(X_select)
            result = select_data.results.pop(idx) # TODO: GP should automatically avoid old samples
            print('Feature:', result['feature'])
