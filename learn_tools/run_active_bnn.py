...
from learn_tools.select_active import active_learning, active_learning_discrete, learning_discrete
...
from learn_tools.active_learner import BEST,CONFIDENT, REJECTION,STRADDLE
from learn_tools.active_nn import ActiveNN, NN_MODELS, RF_CLASSIFIER, get_sklearn_model, RF_REGRESSOR
from learn_tools.active_bnn import ActiveBNN, BNN_MODELS, BNN_REGRESSOR, BNN_CLASSIFIER
...
def create_learner(domain, train_data, algorithm, **kwargs):
    ...
    elif algorithm.name in BNN_MODELS:
        learner = ActiveBNN(domain, initx=X_train, inity=Y_train, initw=W_train,
...
def evaluate_scores(domain, seed, algorithm, learner, num_trials,
    ...
    # simulation robot to get scores
    results = []
    if num_trials == 0:
        return results
    start_time = time.time()
    ...
    trials = get_trials(problem=domain.skill, fn=learner, num_trials=num_trials,
                        seed=seed, visualize=visualize, verbose=serial) #verbose=serial
...
def main():
    ...
    parser.add_argument('-n', '--num_trials', type=int, default=90,
    ...
    # train_sizes = inclusive_range(50, 550, 100)
    #train_sizes = [50]
    #train_sizes = [250]
    # train_sizes = [100]
    ...
    # train_sizes = [200]
    # train_sizes = [150]#inclusive_range(60, 100, 10)
    ...
    # 可视化
    # serial = is_darwin()
    # Todo: Please set serial = True when using neural network model
    serial = False
    ...
    # type of query used to evaluate the learner
    query_type = BEST #STRADDLE #CONFIDENT # ACTIVE #REJECTION # |  |  |
    binary = False
    # Omitting failed labels is okay because they will never be executed
    algorithms = [] #BNN_MODELS BNN_REGRESSOR BNN_CLASSIFIER
    model_names = (BNN_CLASSIFIER,)
    ...
    final_size = 256 # Total train_size when learning is ending
    train_sizes = inclusive_range(init_batch_size[0], final_size, step_size)
    # Passive learning setting
    # Train learner with train_size[N], then evaluate learner
    algorithms += [(Algorithm(bnn_model, variance=True, label='BNN-PL'), inclusive_range(batch_size, final_size, step_size))
                   for bnn_model, batch_size in product(model_names, init_batch_size)]

    # algorithms += [(Algorithm(bnn_model, variance=True, label='BNN-PL'), [num])
    #              for bnn_model, num in product(model_names, train_sizes)]

    # Active learning setting
    # step_size samples are chosen actively and learner is train sequentially
    algorithms += [(Algorithm(bnn_model, hyperparameters=batch_size, variance=True, label='BNN-AL', transfer_weight=tw),
                    inclusive_range(batch_size, final_size, step_size))
                    for bnn_model, tw, batch_size in product(model_names,transfer_weights, init_batch_size)]
    ...
    test_paths = None #SCOOP_TEST_DATASETS # TEST_DATASETS | SCOOP_TEST_DATASETS
    #test_paths = None
    ...
    # =args.num_rounds*trials_per_round
    ...
    for round_idx in range(args.num_rounds):
        seed = seeds[args.num_rounds]# round_idx if args.deterministic else hash(time.time()) # vs just time.time()?
        setup_seed(seed)  # 42
        ...
        for algorithm, active_sizes in algorithms:
            ...
            for active_size in active_sizes:
                ...
                if algorithm.name in BNN_MODELS:
                    learning_discrete(learner, active_data, num_active)
