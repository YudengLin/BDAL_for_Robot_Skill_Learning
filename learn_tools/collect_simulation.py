...
# sys.path.extend([
#     os.path.join(cwd, 'pddlstream/pddlstream'),  # Important to use absolute path when doing chdir
#     os.path.join(cwd, 'ss-pybullet'),
#     os.path.join('.'),
#     os.path.join('./'),
#     os.path.join('../pddlstream'),
#     '../plan_tools'
#
#     # 'pddlstream/examples',
...
from pybullet_tools.utils import ClientSaver, HideOutput, elapsed_time, has_gui, user_input, ensure_dir, \
    create_attachment, wait_for_user, read_pickle, write_json, is_darwin, WorldSaver, VideoSaver, wait_for_duration, \
    write_pickle
...
from active_bnn import get_sklearn_model

def run_trial(args):
    skill, fn, trial, kwargs = args
    time.sleep(trial/5.) # Hack to stagger processes
    # fn.model.model = get_sklearn_model(fn.model_type)
    # with open("1.pkl", 'rb') as file:
    #     fn.model.model = pickle.loads(file.read())

    # fn_t.model.model.load_state_dict(get_weights_copy(fn_t.model.model))
    ...
    seed = hash((kwargs.get('seed'), trial))
    set_seed(seed)

    sim_world, collector, task, feature, evaluation_fn, saver = sample_task(skill, **kwargs)
...
def save_weights(model, weights_path = 'weights_temp.pt'):
    torch.save(model.state_dict(), weights_path)
    return weights_path
def get_trials(problem, fn, num_trials=1, max_time=60, seed=None, valid=True, visualize=False, verbose=False, collect_kwargs={}):
    ...
    # for param in fn.model.model.parameters():
    #     param.requires_grad = False
    # fn.model.model.eval()

    # path = save_weights(fn.model.model.layers)
    # with torch.no_grad():
    #     fn_t = copy.copy(fn)

    write_pickle("fn_tmp.pkl", fn)
    fn_file = "fn_tmp.pkl"

    # output_hal = open("1.pkl", 'wb')
    # str = pickle.dumps(fn.model.model)
    # output_hal.write(str)
    # output_hal.close()
    # fn.model.model = None


    trials = [(problem, fn_file, trial, kwargs) for trial in range(num_trials)]
